...
% --- arrows ---
\draw[->] (ledger) -- node[above,font=\scriptsize]{feeds} (T1);
\draw[->] (T1) -- (T2);
\draw[->] (T2) -- (T3);
\draw[->] (ledger.east |- R) -- ++(0.3,0) |- (rseed.west |- verif);
\draw[->] (T1.east) -- (rseed.west |- T1);
\end{tikzpicture}
...
For readers arriving from outside the HQIV programme, the symbols below recur throughout the text and are defined formally where they first appear:
\begin{description}[leftmargin=*,style=multiline,labelwidth=2.4cm,align=left]
  \item[\(\mathrm{Fin}\,n\)] the finite index type \(\{0,1,\dots,n-1\}\); \(\mathrm{Fin}\,8\) labels octonion channels and \(\mathrm{Fin}\,4\) labels spacetime indices.
  \item[\(A^a{}_\mu,F^a{}_{\mu\nu}\)] discrete gauge potential and field strength on \(\mathrm{Fin}\,8\times\mathrm{Fin}\,4\); \(F^a{}_{\mu\nu}=A^a{}_\nu-A^a{}_\mu\).
  \item[\(m\)] shell index, \(m\in\mathbb{N}\). Calibration shell is \(m_{\mathrm{ref}}=4\) (proton anchor in the QCD-onset ladder).
  \item[\(\rho,K,\Omega\)] discrete densities on the null-shell ladder: weight, curvature numerator, and curvature ratio respectively; \(\Omega_k(m;m_{\mathrm{ref}})\) is the partial curvature ratio at \(m\).
  \item[\(\mathcal{R}\)] normalized cumulative phase readout; \(\delta_{\mathcal{R}}(n)=\mathcal{R}(n+1)-\mathcal{R}(n)\) is the per-shell increment.
  \item[\(\Delta\)] phase-lift generator in the \((e_1,e_7)\) plane of the octonion units; \(\Delta\in\mathfrak{so}(8)\).
  \item[\(G_2\)] the automorphism Lie group of the octonions, \(\mathrm{Aut}(\mathbb{O})\). The notation \(G_2\cup\{\Delta\}\) means the linear span generated by \(G_2\) together with \(\Delta\) under the bracket.
  \item[\(\alpha,\gamma_{\mathrm{HQIV}}\)] curvature imprint and unit-split coefficient; \(\alpha=3/5\), \(\gamma=2/5\), and \((3-\gamma_{\mathrm{HQIV}})=13/5\) is the Friedmann prefactor.
  \item[\(\phi(m)\)] auxiliary scalar tied to the temperature ladder, \(\phi(m)=2(m+1)\); also denoted \(\phi_{\mathrm{val}}\) when used as a parameter of the action cell.
  \item[\(G_{\mathrm{eff}}(\phi)\)] effective gravitational coupling \(\phi^{\alpha}\) entering the HQVM Friedmann identity.
  \item[\(\mathcal{S}_O,\mathcal{S}_{\mathrm{grav}},\mathcal{S}_{\mathrm{tot}}\)] O--Maxwell, gravity, and total action cells (Definition~\ref{def:total}).
  \item[\(v_{\mathrm{lockin}}\)] lock-in vev proxy in the weak/Higgs scaffold, \(v_{\mathrm{lockin}}=\sqrt{\eta_{\mathrm{paper}}\,\Omega_k(m_{\mathrm{lockin}};m_{\mathrm{lockin}})}\).
\end{description}
