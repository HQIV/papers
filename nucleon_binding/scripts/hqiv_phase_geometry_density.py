...
def phase_unit_cell(molecule: str, allotrope: str | None = None) -> PhaseUnitCell:
    """Unit cell from derived allotrope (``hqiv_lab``), not a static table."""
    from hqiv_lab import MaterialsLab

    lab = MaterialsLab()
    spec = lab.spec_from_name(molecule)
    cell = lab.unit_cell(spec, allotrope)
...
def material_scales_with_phase_geometry(
    ...
    *,
    ...
    """Bulk material scales with ρ from unit-cell geometry (not steric proxy alone)."""
    cell = phase_unit_cell(molecule, allotrope)
    rho_g = density_g_cm3(cell)
    rho_frac = curvature_density_fraction(rho_g, molecule)
    ...
    return tptp.MaterialThermodynamicScales(
        ...
        contact_points=4 if molecule.upper() == "H2O" else base.contact_points,
        molecular_weight_amu=cell.molecular_weight_amu,
        intermolecular_contacts=4 if molecule.upper() == "H2O" else base.intermolecular_contacts,
...
def melt_readout_with_phase_geometry(
    ...
    *,
    ...
    """Full witness: geometry → ρ → κ₆ feedback → T_sl @ pressure."""
    cell = phase_unit_cell(molecule, allotrope)
    rho_g = density_g_cm3(cell)
    rho_frac = curvature_density_fraction(rho_g, molecule)
    mat = material_scales_with_phase_geometry(molecule, allotrope=allotrope, bulk=True)
    xi = mat.contact_xi
    fb = hcf.binding_curvature_feedback_second_order_homogeneous(
        xi, rho_frac, coordination_excess=0.0
    )
    ...
    return {
        ...
        "B_eff": hcf.effective_curvature_budget(xi, rho_frac),
    }
