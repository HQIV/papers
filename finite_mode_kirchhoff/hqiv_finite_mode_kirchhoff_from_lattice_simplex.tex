...
\date{Published v1 --- Zenodo \href{https://doi.org/10.5281/zenodo.20416564}{10.5281/zenodo.20416564} (2026-05-27)}
...
Tier-0 records and the preceding Tier-1 companions
\cite{hqiv-3d-growth-paper,hqiv-oct-action-paper} carry minted Zenodo DOIs.
...
sole dimensionful anchor for the mass chart (the
\hyperref[lean:proton-lockin]{proton lock-in} convention, recorded in
the \hyperref[lean:scale-witness]{scale-witness} formalism of
\cite{hqiv-lean}).
...
Appendix~\ref{app:scripts}, packaged as \texttt{scripts.zip} on the Zenodo record
...
\item \textbf{One dimensionful anchor.}  The HQIV formalisation
  \cite{hqiv-lean} allows only one dimensionful calibration per
  phenomenology pass; we take the default
  \hyperref[lean:proton-lockin]{proton lock-in}.  At shell
  $m=4$ the proton mass follows the internal
  constituent-plus-binding readout below, calibrated to the
  measured value $M_p=938.272\,\mathrm{MeV}$ to fix the mass/unit
  chart.  The shell index $m=4$ is the QCD-onset identification
  (not a free parameter), and the binding closure at that shell is
  a non-trivial HQIV calculation, but the MeV scale itself is an
  \emph{anchor}, not an independent two-axiom output.
  Section~\ref{sec:proton} states what is and is not claimed.
...
\paragraph{Proton lock-in as the mass chart.}
The HQIV library \cite{hqiv-lean} is written so that a given
phenomenology pass fixes \emph{one} dimensionful scale before
derived readouts are compared to experiment.  We use the default,
\hyperref[lean:proton-lockin]{proton lock-in}: at shell $m=4$ the
proton mass comes from the internal constituent-plus-binding
readout in the next paragraph, and the measured value
$938.272\,\mathrm{MeV}$ is the calibration that sets the mass/unit
chart for this paper---not a parameter we adjust elsewhere in the
derivation.  Once that chart is fixed, CODATA $1/\alpha$, CMB
horizon scales, and PDG centrals \cite{pdg2024} enter only as
predictions or comparison checks.  The
\hyperref[lean:scale-witness]{scale-witness} enum in Lean makes the
same rule explicit: proton mass, fine-structure data, and a separate
cosmological scale must not be treated as simultaneous inputs unless
the active anchor is stated.
