...
This is the fourth paper in the HQIV preprint series and the
first of the Tier-2 Standard-Model layer. Tier 0 fixes the
framework (HQIV main paper~\cite{ettinger2026hqiv}, Tier-0a) and
the discrete null-lattice readout (lightcone
paper~\cite{hqiv-lightcone-paper}, Tier-0b; closure
paper~\cite{hqiv-so8-paper}, Tier-0c). Tier 1 wraps that spine
with the dimensional rigidity of three-dimensional causal growth
\cite{hqiv-3d-growth-paper}, the variational layer
\cite{hqiv-oct-action-paper}, and the finite-mode Kirchhoff law
\cite{hqiv-kirchhoff-paper} on the same lattice. The present
paper rides every one of those upstream results to derive an
electroweak scale and a fermion mass ladder from horizon
readouts alone---no continuum quantum field theory imported, no
Standard-Model Lagrangian assumed in advance. The Tier-0 records
carry minted Zenodo DOIs; the 3D causal-growth companion
\cite{hqiv-3d-growth-paper} is published on Zenodo; the remaining
Tier-1 records cited above are pre-DOI (co-deposited as the Lean PR
builds clear).
