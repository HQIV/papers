...
def integrate_window_stratified(
    ...
    total_w = sum(weights)
    D_over_H = sum(v * w for v, w in zip(d_vals, weights)) / total_w
    ...
    if coupled_readout is not None:
        ...
        D_bare_uncoupled = sum(s.D_over_H * w for s, w in zip(bare_sto, weights)) / total_w
...
def run_bbn_integrator(cfg: BBNIntegratorConfig | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "synthesis_d_window": {
            ...
            "notes": (
                "D/H log-weighted over freeze→C₂ bottleneck; n+p→D capture + strong deposition "
