...
def synthesis_window_end_mev(eta: float, Q_np: float) -> float:
    """End of active n+p / D / α synthesis (C₂ bottleneck to mid-MeV gate)."""
    return max(lean.bbn_dynamic_c2_bottleneck_t_mev(eta, Q_np), 0.15)
...
def synthesis_d_window_bounds_mev(
    ...
    *,
    ...
    """
    D/H integration window: MeV synthesis band through the C₂ bottleneck.

    Excludes the 0.01 MeV tail where ``thermal_sink(D,α,T)`` crushes the partition
    seed; keeps the freeze→bottleneck band where n+p→D and D burn compete.
    """
