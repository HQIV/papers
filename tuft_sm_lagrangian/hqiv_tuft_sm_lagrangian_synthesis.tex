...
\noindent\textbf{Status.} Proved at the patch level: Hopf shells, $\kappa_6$ factorisation ($C_2=56/45$ at lock-in), five sector equalities, Yukawa linearity. Not claimed: full anomaly cancellation, measure choice, complete non-abelian holonomy, or subleading zeta terms. Details: Table~\ref{tab:claim-status}, Appendices~\ref{app:lean-catalog}--\ref{app:lean-witnesses}.
\end{abstract}
\maketitle
...
This paper combines the programmes: \textbf{Nielsen's TUFT supplies the topological classifying space and synthesis vision; HQIV supplies the dynamic, finite, machine-checked realisation and the embedding into the full Standard Model Lagrangian.} The construction is proved at the patch level---smooth-manifold $\mathcal{L}_{\mathrm{SM}}$ notation is a cross-citation rewrite (§\ref{sec:background}). Technical identifiers for reproducibility are deferred to the appendices.

The pipeline is summarised in Figure~\ref{fig:pipeline}. Table~\ref{tab:claim-status} separates proved identities from named-but-unclaimed extensions. Single-witness discipline and the proton$\to$cosmos propagation chain are in \S\ref{subsec:scale-witness}; an error budget for the PDG comparisons is in \S\ref{sec:error-budget}; sharp experimental channels in \S\ref{sec:falsifiability}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{0.34\linewidth}>{\raggedright\arraybackslash}p{0.12\linewidth}>{\raggedright\arraybackslash}p{0.40\linewidth}}
    ...
    Full anomaly cancellation & Named, not claimed & --- \\
    Higher-order zeta / holonomy upgrade & Neglected or open & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Not claimed:} full anomaly cancellation; measure choice; complete non-abelian holonomy; subleading zeta terms. The ontology is the finite patch layer---a literal continuum rewrite would erase the discrete index carrying the predictions.
...
The discrete patch layer is the ontology. Smooth-manifold textbook notation is a calculation-approximation rewrite for cross-citation only.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    ...
    \node[below=0.45cm of disc, lbl, text width=5.2cm] {constructed witness on the discrete patch; no continuum limit assumed};
...
\item \textbf{Finite vs smooth carrier.} All load-bearing equalities are patch-level witnesses on $\mathrm{Fin}\,8$ (and related finite index sets). A literal continuum limit is neither assumed nor proved; smooth $\mathcal{L}_{\rm SM}$ notation is a cross-citation rewrite (patch-ontology contract in the abstract).
...
\item \textbf{Scope boundaries.} Lean certifies the structural bridge and readout arithmetic with zero \texttt{sorry}; it does \emph{not} claim full anomaly cancellation, measure choice, complete non-abelian holonomy, or retained higher-order zeta terms in the determinant expansion.
\end{itemize}
...
The present synthesis establishes the structural bridge and the dynamic mechanism at the abelian projection level and with leading zeta terms. It does not claim a complete derivation of all Standard-Model parameters from the two axioms alone without any scale witness; the default witness is one high-precision local mass ($m_p$), from which cosmological ages and $T_{\mathrm{CMB}}$ follow (\S\ref{subsec:scale-witness}). Higher-order zeta terms, full anomaly cancellation, measure choice, and the complete non-abelian holonomy upgrade remain outside scope (see the scope box in \S1 and Table~\ref{tab:claim-status}).
