...
class LabOutsideEnvironment:
    ...
    * **CMB dipole** — peculiar ``v/c`` books an additive lapse slot on the same ``G_eff``
    ...
    def outside_closure_k_mass_chart(self) -> float:
        """``K`` on the hadronic mass chart at ``ξ_lock`` (gravity well only; ``B_curv=1``)."""
        return outside_closure_K_at(
            self.anchor_xi,
            self.combined_phi_epsilon,
            xi_lock=self.anchor_xi,
        )

    def outside_closure_k_ambient(self) -> float:
        """``K = B_curv(ξ_ambient) × f_g(ε)`` — universe-age slot + gravity well."""
        return outside_closure_K_at(
            self.lab_xi_from_temperature,
            self.combined_phi_epsilon,
            xi_lock=self.anchor_xi,
    ...
    def outside_closure_k_cmb(self) -> float:
        """``K`` at the CMB monopole temperature (relic universe-age chart)."""
        return outside_closure_K_at(
            self.reference_xi_from_temperature,
            self.combined_phi_epsilon,
            xi_lock=self.anchor_xi,
        )
...
class OutsideClosureK:
    """
    Named Earth-scale outside-closure factor for laboratory readouts.

    ``K(ξ, ε) = B_curv(ξ) × f_g(ε)`` where:

    * **Universe age** — ``ξ = T_Pl/(k_B T)`` maps ambient or CMB temperature to
      ``B_curv(ξ)`` (Casimir local/global budget; unity at ``ξ_lock``).
    * **Gravity well** — ``ε = Σ GM/(Rc²) + v_dipole/c`` folds through
      ``f_g = 1 + γ((1+ε)^α − 1)`` (α = 3/5, γ = 2/5).

    Used explicitly in relic-ν opacity and weak-width catalysis; mass chart at
    ``ξ_lock`` keeps ``B_curv = 1`` so only ``f_g`` contributes (~0.3 ppm on Earth).
    Width / lifetime charts use the full ``K`` at lab ``ξ``.
    """
    ...
    K_cmb: float
    K_imprint: float
    temperature_modulator_ambient: float
    outside_modulator_ambient: float
    increment_vs_anchor: float
    support_ratio_vs_cmb: float
    gravity_phi_epsilon: float
    cmb_proper_motion_v_over_c: float
    gravity_stack: dict[str, float]

    def to_dict(self) -> dict[str, float | dict[str, float]]:
        return asdict(self)


def outside_closure_b_curv(xi: float, *, xi_lock: float = XI_LOCKIN) -> float:
    """``B_curv(ξ)`` — universe-age / temperature slot (κ₆ matter-chart budget)."""
    return lean.curvature_budget_at_xi(xi, xi_lock)
...
def outside_closure_K_at(
    ...
    *,
    ...
    """Unified outside closure: ``B_curv(ξ) × f_g(ε)``."""
    return outside_closure_b_curv(xi, xi_lock=xi_lock) * outside_gravity_geff_modulator(
        phi_gravity_epsilon
    )
...
def earth_outside_closure_k(
    ...
    """Export decomposed ``K`` for an Earth-surface lab (default: 300 K, full gravity stack)."""
    env = env or LabOutsideEnvironment()
    xi_lock = env.anchor_xi
    xi_amb = env.lab_xi_from_temperature
    xi_cmb = env.reference_xi_from_temperature
    b_lock = outside_closure_b_curv(xi_lock, xi_lock=xi_lock)
    b_amb = outside_closure_b_curv(xi_amb, xi_lock=xi_lock)
    b_cmb = outside_closure_b_curv(xi_cmb, xi_lock=xi_lock)
    f_grav = env.gravity_modulator()
    f_kin = env.kinetic_modulator()
    f_combined = outside_gravity_geff_modulator(env.combined_phi_epsilon)
    k_mass = b_lock * f_combined
    k_amb = b_amb * f_combined
    k_cmb_slot = b_cmb * f_combined
    return OutsideClosureK(
        ...
        K_ambient=k_amb,
        K_cmb=k_cmb_slot,
        K_imprint=max(k_amb - 1.0, 0.0),
        temperature_modulator_ambient=env.temperature_modulator(at_lab=True),
        outside_modulator_ambient=env.lab_modulator(),
        increment_vs_anchor=env.increment_vs_anchor(),
        ...
        gravity_stack=local_lab_gravity_binding_stack(env.gravity_tier).as_dict(),
    )
...
def apply_outside_closure_k_to_outside_binding(
    ...
    *,
    env: LabOutsideEnvironment | None = None,
    chart: Literal["mass", "ambient"] = "ambient",
) -> float:
    """
    Scale an outside-ledger binding row by the Earth ``K`` factor.

    * ``mass`` — ``ξ_lock`` chart (gravity well only).
    * ``ambient`` — full ``B_curv(ξ_lab) × f_g`` for width / lifetime pipelines.
    """
    env = env or LabOutsideEnvironment()
    k = (
...
def nuclear_binding_conditions_witness(
    *,
    ...
    return {
        ...
        "earth_outside_closure_K": earth_outside_closure_k(
            LabOutsideEnvironment(gravity_tier="full")
        ).to_dict(),
    }
