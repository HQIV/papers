...
publication sequence). Tier~0 fixes the framework
...
are published on Zenodo. The present note is the natural variational sequel:
every sector of the textbook Standard-Model Lagrangian
$\mathcal{L}_{\rm SM}$ is identified with projections of the HQIV
\emph{discrete} octonion action already proved upstream. The dynamic
TUFT/Hopf mass chart that supplies Yukawa weights at a chosen now slice
is the immediate Tier-2 companion
\cite{hqiv-tuft-dynamic-mass-spectrum-paper} (pre-DOI at this paper's
publication; sequence-strict deposit follows).
...
Higgs kinetic and potential from the octonion-scalar norm evaluated
on the dynamic vacuum expectation value; Yukawa weights from the
TUFT/Hopf mass chart at the chosen now slice (structural slot proved
here; phenomenological readout in the companion). The headline theorem
packages all five sector equalities simultaneously. The topological
suppression in the electroweak mass chart factorises as
$\kappa_6(\xi,\Phi,t)=\eta_{\mathrm{local}}(\xi)\,\gamma\,C_2(\xi,\Phi,t)$:
homogeneous matter fraction $\eta_{\mathrm{local}}=\eta_{\mathrm{paper}}$
(same-epoch curvature budget $B_{\mathrm{curv}}=1$ through recombination),
overlap $\gamma=2/5$, and second-order lapse
concentration $C_2$ from $\delta$-corrected Rindler detuning at the
readout point (proved $C_2=56/45$ at $\xi_{\mathrm{lock}}$ with
$\Phi=t=0$). Because the effective vev and $\kappa_6$ are explicit
functions of the selected now slice---via inner--outer Casimir balance,
the curvature primitive, and HQVM lapse data---the entire Lagrangian at
any epoch is determined geometrically once that slice is chosen. There
are no remaining free parameters in the conventional sense: no fitted
Yukawa matrix, no tuned $C_2$, and no simultaneous PDG mass solve. The
combinatorial lattice fixes $\alpha=3/5$, $\gamma=2/5$,
$\alpha_{\rm GUT}=1/42$; the only operational choice is a consistent
``now'' slice $(\xi,\Phi,t)$ on the temperature ladder. PDG centrals
and CODATA couplings are comparison layers only. Reader-friendly Lean
anchors are in Appendix~\ref{app:lean-index}.
...
resonance ladder, $\kappa_6$ factorisation, and the rationals above) a
literal continuum rewrite would erase the discrete index doing the
predictive work.

\noindent\textbf{Status.}
What is proved: the five sector equalities, the total-action
split, and Yukawa linearity over flavours, all under the
dynamic inner--outer Casimir scale. The theory now carries
zero free parameters beyond the discrete lattice data; the
only operational choice is the identification of a consistent
``now'' slice on the temperature ladder. What is named explicitly
and \emph{not} claimed: anomaly cancellation, measure choice,
and the full non-abelian holonomy upgrade (scaffolded upstream
but not closed here).
\end{abstract}
\maketitle
...
\subsection{No free parameters; only now-slice selection}

With the inner--outer Casimir dynamics on the same Hopf-shell carrier
and the curvature primitive along the physical-temperature ladder
in place, the construction contains no free parameters in the
conventional sense. The combinatorial data of the lattice fix
the dimensionless rationals $\alpha=3/5$, $\gamma=2/5$ and
$\alpha_{\rm GUT}=1/42$ once and for all. The only operational
choice in any given solve or phenomenological export is the
selection of a consistent ``now'' slice on the temperature ladder.
Once that slice is identified, the dynamic scale determines the
effective vacuum expectation value at that epoch, the resonance
ladder supplies the Yukawa weights, and the sector projections of
the discrete octonion action yield the full Lagrangian. Different
choices of now slice generate different effective spectra and
couplings; the geometry supplies the explicit temperature evolution
that relates them. The proton lock-in at $\mathrm{referenceM}=4$
remains a natural calibration point for the hadronic sector, but
it is no longer required for the leptonic or gauge sectors.

\subsection{Mass-scale closure: local matter fraction and lapse concentration}
\label{subsec:kappa6}

The TUFT/Hopf bridge of \cite{hqiv-tuft-dynamic-mass-spectrum-paper} supplies
the electroweak mass chart
$\Lambda_{\mathrm{Hopf}}(\xi)=\sqrt{2\pi}\,v(\xi)\,\kappa_6(\xi,\Phi,t)$.
The former fitted slot $C_2\approx 1.135$ is replaced in Lean by a derived
readout (zero \texttt{sorry} in
...
= \eta_{\mathrm{paper}}\,B_{\mathrm{curv}}(\xi),
\qquad
B_{\mathrm{curv}}(\xi)=1
\quad\text{(homogeneous era through recombination).}
\end{equation}
The cumulative chart ratio $\Omega_k(\xi)/\Omega_k(\xi_{\mathrm{lock}})$ compares
epochs to lock-in; it is \emph{not} the same-epoch local/global budget.
While local and global curvature coincide (through recombination and the
lock-in chart), $B_{\mathrm{curv}}=1$ and $\eta_{\mathrm{local}}=\eta_{\mathrm{paper}}$
(\hyperref[lean:eta-local-shell]{homogeneous matter fraction}).
The partial-horizon readout $\eta(n;N)=\eta_{\mathrm{paper}}\cdot\Omega_k(n;N)$
remains the baryogenesis witness for shell-$n$ vs horizon-$N$ comparisons only.
The overlap $\gamma_{\mathrm{HQIV}}=2/5$ is the proved horizon-split
coefficient (\hyperref[lean:gamma-25]{horizon-split $\gamma$ identity}).
The second-order factor is \emph{lapse concentration} at the readout
point:
\begin{equation}
\label{eq:C2-lapse}
C_2(\xi,\Phi,t)
= \frac{1+\gamma}{2}\,
\frac{\texttt{rindlerDenWithDelta}\bigl(\lambda\,\mathrm{obs}(\xi,\Phi,t),\,\mathrm{referenceM}\bigr)}
{\texttt{rindlerDenShared}(\mathrm{referenceM})},
\end{equation}
with $\lambda=c_{\mathrm{rindler}}=\gamma/2$ and
$\mathrm{obs}(\xi,\Phi,t)=\Theta_{\mathrm{local}}(\xi)\,(1+\gamma)+(N-1)$
for $N=\texttt{HQVM\_lapse}(\Phi,\varphi(\xi),t)$.
This packages the same $\delta$-corrected detuning observable used in
\cite{hqiv-oct-action-paper} and the global-detuning module: matter
curvature is corrected \emph{locally} by $\Omega_k(\xi)$; observer
concentration enters through $C_2$, not through a hidden Yukawa fit.

At lock-in ($\xi_{\mathrm{lock}}=5$, $\Phi=t=0$, $\mathrm{referenceM}=4$)
Lean proves $C_2=56/45$ (\hyperref[lean:C2-lockin]{lock-in lapse concentration}).
The pre-closure regression constant $1.135136\ldots$ is retained only as
\texttt{tuftHopfKappa6SecondOrderCorrectionLegacy} for chart comparison.
The physical $\kappa_6$ used by neutrino and lepton readouts is
\texttt{tuftHopfKappa6AtXi} $\xi\,\Phi\,t$; at lock-in it is
\texttt{tuftHopfKappa6} (\hyperref[lean:kappa6-at-xi]{$\kappa_6$ at horizon}).
\textbf{BBN calibrates dynamic $C_2$:} on the hot chart,
$\kappa_6(\xi)=\eta_{\mathrm{paper}}\,B_{\mathrm{curv}}(\xi)\,\gamma\,C_2(\xi)$
grows with $\xi=T_{\mathrm{Pl}}/T$; the MeV cooling network
(\cite{hqiv-baryogenesis-paper,hqiv-bbn-paper}) applies
$T_{\mathrm{bn}}=\gamma w_{\mathrm{strong}} T_{\mathrm{freeze}}(\eta)$ and
$w_{C_2}(T)=\gamma w_{\mathrm{strong}} Q_D^{\mathrm{eff}}(T)/Q_{np}$ in
$(\kappa_{6,\mathrm{ref}}/\kappa_6)^{w_{C_2}}$---the lapse reaction clock, not a
raw $\kappa_6$ multiplier.

For the SM Lagrangian assembly, the lock-in vev
(\hyperref[lean:lockin-vev]{lock-in vev}) remains the weak-sector scalar
anchor $\sqrt{\eta_{\mathrm{paper}}}$ when $\Omega_k=1$ at lock-in, while
charged-lepton masses at epoch $\xi$ follow the dynamic vev/TUFT chart of
\cite{hqiv-tuft-dynamic-mass-spectrum-paper}. The five sector equalities of
Theorem~\ref{thm:headline} are unchanged; what changes is that the mass
\emph{normalization} feeding $\mathcal{L}_Y$ is no longer a separate fit
layer once $(\xi,\Phi,t)$ is fixed.
...
$\mathcal{L}_O+S_{\rm HQVM\,grav}$. \cite{hqiv-tuft-dynamic-mass-spectrum-paper} adds the
...
\item \textbf{Mass-scale closure} (Sect.~\ref{subsec:kappa6}):
  $\kappa_6(\xi,\Phi,t)=\eta_{\mathrm{local}}(\xi)\,\gamma\,C_2(\xi,\Phi,t)$
  with local $\eta$, proved lapse concentration at lock-in, and
  \hyperref[lean:kappa6-factorisation]{$\kappa_6$ factorisation witness}.
\item \textbf{Yukawa from the TUFT mass chart} (Sect.~\ref{sec:yukawa}):
  $y_f := \sqrt{2}\,m_f(\xi)/v(\xi)$ with $m_f$ from the Hopf/Beltrami
  scalar at the now slice; the identity $y_f v / \sqrt{2}=m_f$ is proved
  (the \hyperref[lean:yukawa-mass]{Yukawa--mass identity}).
...
\item \textbf{Parameter witness} (Sect.~\ref{sec:parameters}):
  the \hyperref[lean:param-witness]{parameter witness} certifies
  $\alpha=3/5$, $\gamma=2/5$, $\alpha_{\rm GUT}=1/42$; the
  $\kappa_6$ factorisation replaces every former fitted second-order
  slot---not a simultaneous PDG fit set.
\end{itemize}
...
\item \textbf{Single witness, no PDG fit.} Yukawa weights come from the
  ...
  \cite{hqiv-tuft-dynamic-mass-spectrum-paper,hqiv-kirchhoff-paper}, PDG masses are
...
m_f(\xi)/v(\xi)$ at the selected now slice, with
$v(\xi)=\texttt{tuftVevAtXi}(\xi)$ from the dynamic Casimir chart and
$m_f(\xi)$ from the TUFT Hopf/Beltrami scalar
(\cite{hqiv-tuft-dynamic-mass-spectrum-paper},
\hyperref[lean:tuft-lepton-mass]{TUFT lepton mass at $\xi$}).
At lock-in the archived geometric ladder in
\texttt{papers/archive/lean\_to\_mass\_spectrum/} remains available as a
diagnostic, but the physical chart uses $\Lambda_{\mathrm{Hopf}}=\sqrt{2\pi}\,v\,\kappa_6(\xi,\Phi,t)$
with $\kappa_6$ from \eqref{eq:kappa6-factorisation}---not a separate
Yukawa fit.
...
fitted from experiment. In HQIV these collapse to: three lattice rationals,
one now slice $(\xi,\Phi,t)$, and the proved factorisation
\eqref{eq:kappa6-factorisation}--\eqref{eq:C2-lapse}. Laboratory masses
enter only as comparison targets. The baryogenesis anchor
$\eta_{\mathrm{paper}}$ is the same surplus wired into
$\eta_{\mathrm{local}}(\xi)$ and into \texttt{lockinVev}; the dynamic
bulk integrator (\texttt{scripts/hqiv\_dynamic\_bulk\_bbn.py}) propagates
$\eta_{\mathrm{local}}$ shell-by-shell and reports emergent lock-in without
re-fitting $\eta$ at each step.
\end{remark}
...
\cite{hqiv-tuft-dynamic-mass-spectrum-paper,hqiv-kirchhoff-paper}.

\begin{table}[ht]
\caption{Inputs to the HQIV-derived SM Lagrangian after the $\kappa_6$
closure. None is a simultaneous PDG fit; the only operational choice is the
now slice $(\xi,\Phi,t)$.}
...
\midrule
$\alpha=3/5$ & Lattice stars-and-bars &
  \hyperref[lean:alpha-35]{lattice $\alpha$ identity} \\
$\gamma=2/5$ & Horizon split $1-\alpha$ &
  \hyperref[lean:gamma-25]{horizon-split $\gamma$ identity} \\
$\alpha_{\rm GUT}=1/42$ & Cube $\times$ octonion channels &
  \hyperref[lean:alpha-gut]{GUT coupling identity} \\
$\eta_{\mathrm{local}}(\xi)$ & $\eta_{\mathrm{paper}}\,B_{\mathrm{curv}}(\xi)$, $B_{\mathrm{curv}}=1$ &
  \hyperref[lean:eta-local-shell]{homogeneous matter fraction} \\
$C_2(\xi,\Phi,t)$ & Lapse concentration \eqref{eq:C2-lapse} &
  \hyperref[lean:C2-lockin]{lock-in $C_2=56/45$} \\
$\kappa_6$ & $\eta_{\mathrm{local}}\,\gamma\,C_2$ &
  \hyperref[lean:kappa6-factorisation]{factorisation} \\
$v(\xi)$, $m_f(\xi)$ & Dynamic Casimir + TUFT scalar &
  \hyperref[lean:tuft-lepton-mass]{TUFT masses at $\xi$} \\
$\lambda_{\rm eff}$ & EW-shell quartic &
  \hyperref[lean:lambda-eff]{effective quartic} \\
\texttt{lockinVev} & $\sqrt{\eta_{\mathrm{paper}}}$ at $\Omega_k=1$ &
  \hyperref[lean:lockin-vev]{lock-in vev} \\
\bottomrule
\end{tabular}
\end{table}
...
bridge compresses them to: three proved lattice rationals, one now slice
$(\xi,\Phi,t)$, and the derived $\kappa_6$ factorisation. Gauge couplings
trace to $\alpha_{\rm GUT}$ and the $\phi$-correction of
\cite{hqiv-oct-action-paper}; charged-fermion masses trace to the TUFT chart;
$(\mu^2,\lambda)$ trace to $(\eta_{\mathrm{paper}},\lambda_{\rm eff})$ at
lock-in. The dynamic baryogenesis integrator recovers
$\eta_{\mathrm{paper}}$ at lock-in to $\sim 1\%$ on the $\Omega_b$ path
without injecting a separate $\eta$ knob per shell.
\end{remark}
...
None of these tasks requires new physics input. The discrete constants,
$\kappa_6$ factorisation, dynamic vev, and mass chart are already proved;
smooth-manifold form is a translation layer only---not a continuum-completion
programme.
...
\item[\phantomsection\label{lean:kappa6-factorisation}\textbf{$\kappa_6$ factorisation.}]
  \leanid{tuftHopfKappa6\_eq\_matter\_fraction\_gamma\_lapse\_concentration}
  in \leanfile{Hqiv/Physics/HopfShellBeltramiMassBridge.lean}.
\item[\phantomsection\label{lean:eta-local-shell}\textbf{Local matter fraction on shells.}]
  \leanid{tuftMatterFractionAtXi\_eq\_eta\_partial\_on\_shell}.
\item[\phantomsection\label{lean:C2-lockin}\textbf{Lock-in lapse concentration.}]
  \leanid{tuftLapseConcentrationAtXi\_lockin\_zero}.
\item[\phantomsection\label{lean:kappa6-at-xi}\textbf{$\kappa_6$ at horizon.}]
  \leanid{tuftHopfKappa6AtXi}, \leanid{tuftHopfKappa6}.
\item[\phantomsection\label{lean:tuft-lepton-mass}\textbf{TUFT lepton mass at $\xi$.}]
  \leanid{tuftLeptonMassFromVevAtXi\_MeV}.
\end{description}
\endgroup
